...
for (const [name, data] of Object.entries(grouped)) {
  ...
  console.log(`│  Peak day  : ${peak.date} → ${peak.views} views, ${peak.uniques} uniques`);
  console.log(`│`);
  console.log(`│  Date          Views  Uniques`);
  data.slice(0, 10).forEach((r) =>
    console.log(`│  ${r.date}    ${String(r.views).padStart(5)}  ${String(r.uniques).padStart(7)}`)
  );
  console.log(`└${"─".repeat(40)}`);
}

// Last sync logs
const logs = db.prepare(`SELECT * FROM sync_log ORDER BY synced_at DESC LIMIT 5`).all();
console.log("\n🔄  Recent syncs:");
logs.forEach((l) =>
  console.log(`   ${l.synced_at.slice(0, 19)}Z  [${l.status.padEnd(5)}]  ${String(l.entries).padStart(3)} entries  — ${l.repos}`)
);
